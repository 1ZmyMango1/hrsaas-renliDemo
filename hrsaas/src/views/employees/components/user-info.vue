      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <ImageUpload ref="avatarRef"></ImageUpload>
            <!-- 放置上传图片 -->
          </el-form-item>
        </el-col>
      </el-row>
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <ImageUpload ref="percentRef"></ImageUpload>
          <!-- 放置上传图片 -->
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 保存下半部分信息
    async savePersonal() {
      const fileList = this.$refs.percentRef.fileList;
      // 判断图片是否全部上传完毕  --- upload是否存在
      // 是否包含未未上传完毕的元素
      if (fileList.some((item) => !item.upload)) {
        // 错误提示
    ...
    async getPersonalDetail() {
      ...
      // 通过ref获取到子组件的实例，然后获取到filelsit进行赋值
      // 获取到头像数据，已经是上传完成的，直接可以提交，所以打一个upload标记
      this.$refs.percentRef.fileList = [
        {
